...
qtile_config = f"{config}/qtile"
scripts = f"{qtile_config}/scripts"
wallpapers = f"{qtile_config}/wallpapers"
...
keys = [
    ...
    # Lock screen
    Key([sup, alt], "l", lazy.spawn(f"{qtile_script} lock {wallpapers}"), desc="Lock screen"),
    ...
    Key([sup], "t", spawn_or_focus("teams-for-linux")),
...
groups = [
    Group("1", layout="bsp"),
    Group("2", layout="bsp"),
    Group("3", layout="bsp", matches=[Match(wm_class=["brave-browser"])]),
    Group("4", layout="bsp", matches=[Match(wm_class=["teams-for-linux"])]),
    Group("5", layout="bsp"),
    Group("6", layout="bsp"),
